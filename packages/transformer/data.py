...
def test_cora_data_gen(adj: torch.Tensor, features: torch.Tensor, test_nids: torch.Tensor, labels: torch.Tensor, device: str):
    ...
    return TransformerGraphBundleInput(features.unsqueeze(0), labels[test_nids].unsqueeze(0), adj_mat_layerwise.unsqueeze(0) == 1, test_nids.unsqueeze(0), test_nids.shape[0], device)
