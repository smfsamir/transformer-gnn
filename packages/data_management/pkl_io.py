...
def store_pic_dynamic(plt, fname, root_folder='results', use_pdf=False):
    date = datetime.today().strftime('%Y-%m-%d')
    folder = "{}/{}/".format(root_folder, date)
...
def load_npy(path_fname_concat):
    with open(path_fname_concat, 'rb') as f: 
        arr = np.load(f)
    return arr

def save_npy(path_fname_concat, obj):
    with open(path_fname_concat, 'wb') as f: 
        np.save(path_fname_concat, obj)
